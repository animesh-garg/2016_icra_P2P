...
\fp{cite springer handbook of robotics which has a nice chapter on LfD, 
    also not all learning from segmentation is concerned with segmentation 
    - many only work under the assumption that motion is already segmented}
...
\fp{Instead, N et al use visual information for...} Post segmentation,  Niekum et al. uses AR markers to estimate object poses of all of the objects in the workspace.
...
Calinon et al.~\cite{calinon2014skills, calinon2014task, calinon2010learning} \fp{I think A. Billard is another key
leader here} characterizes segments from demonstrations as skills that can be used to parametrize imitation learning.
...
\fp{vague. Instead: where segmentation is challenging due to X, Y, Z (cite).}
...
optimization such as skeleton or action models using in human action recognition. \fp{The whole Deep learning aspect
needs to be more promionent earlier on as key enabling factor}
...
There are a number of different ways to formalize segmentation, and we overview \fp{`review' or `provide an overview'} some of the prior work (including ours) to contextualize the results in the rest of the paper.
